...
unsigned int nStakeMaxAge = 60.0 * 60.0 * 24.0 * 14.0;   // 14 days       
...
const string strMessageMagic = "uTrump Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "uTrump", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Fathead bans Muslims from 7 countries: 28 Jan 2017";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1485631562;
        }
        else
        {
            txNew.nTime    = 1485631558; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015.
        }
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1485631564;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1553379;
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0xff7a830b217af158c15d4210905bdd1584f83b90d2ee0b3dcbdc59afd6857d05"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb2, 0xf5, 0xc3, 0xe0 };
